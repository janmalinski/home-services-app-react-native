...
export const MapScreen = ({ route, navigation }: Props) => {
  ...
  const hasLocationPermissionIOS = async () => {
    ...
    if (status === 'disabled') {
      Alert.alert(i18n.t('add:locationDisabled'), '', [
        { text: i18n.t('add:goToSettings'), onPress: openSettings },
        {
          text: i18n.t('add:dontUseLocation'),
          onPress: () => {
            console.log('USE OF LOCATION DISABLED BY USER');
          },
        },
      ]);
    }
  ...
  const hasLocationPermission = async () => {
    ...
    if (status === PermissionsAndroid.RESULTS.DENIED) {
      ToastAndroid.show(i18n.t('add:locationPermissionDenied'), ToastAndroid.LONG);
    } else if (status === PermissionsAndroid.RESULTS.NEVER_ASK_AGAIN) {
      ToastAndroid.show(i18n.t('add:locationPermisssionRevoked'), ToastAndroid.LONG);
    }
    return false;
  };
            ...
            <View style={[styles.container, styles.questionContainer]}>
              <StyledText style={styles.questionText}>
                {userType.name === 'Client'
                  ? i18n.t('map:whereAreYouLookingForHelp')
                  : i18n.t('map:whichAreYouWantToWork')}
              </StyledText>
            </View>
